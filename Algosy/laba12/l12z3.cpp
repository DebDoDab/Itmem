...
#define ft first
#define sd second
#define openfiles ifstream cin ("decomposition.in"); ofstream cout ("decomposition.out");
#define pb push_back
...
signed main()
{
    ...
    function<int(int, int)> dfs2 = [&](int x, int f) {
        if (x == t) {
        ...
        for (auto& e: G[x]) {
            auto& edge = Edges[e];
            if (edge.f > 0) {
                int flow = dfs2(edge.b, min(f, edge.f));
                if (flow) {
                    Ans.back().push_back(edge.i);
                    if (x == s) {
                        int tempSize = Ans.back().size();
                        Ans.back().push_back(tempSize);
                        Ans.back().push_back(flow);
